 ...
 * @property {string[] | undefined} recurrenceSkipStarts
  ...
  /** ???ºÏ†ï(?†ÏßúÎß???Í≤ΩÏö∞)?Ä ?∞ÏÉâ ?åÎßà ?†Ï?, ??™© ?†ÌÉù¬∑?ÅÌÉú Î≥ÄÍ≤??úÏóê???ÅÌÉú??*/
  ...
  /** ?†Ìò∏ Î™®Îç∏ ?úÏÑú (?§Ï†ú Í∞Ä??Î™®Îç∏Í≥?ÍµêÏßë?©ÏúºÎ°??†ÌÉù) */
  ...
  /** ???ºÏ†ï ?ëÏÑ±???ÑÏãú ?ÅÌÉú/Ï§ëÏöî??*/
  let draftStatus = "ready";
  let draftImportance = "medium";
  /** ?∞Ï∂úÎ¨??ÖÎ†•?âÏóê???±Í∏â??ÏßÅÏ†ë Í≥†Î•∏ ?ÅÏù¥ ?àÎäîÏßÄ (Í≥†Î•¥Í∏??ÑÏóê???åÏÉâ Î∞ïÏä§) */
  let deliverableHeadImpPicked = false;
  /** ?∞Ï∂úÎ¨??±Î°ù ?†ÎãàÎ©îÏù¥??ÏßÑÌñâ Ï§ëÏù¥Î©?Ï§ëÎ≥µ ?±Î°ù??ÎßâÎäî??*/
  ...
  // ?¥Ï†Ñ ?§Ìñâ?êÏÑú ?®ÏïÑ?àÏùÑ ???àÎäî Ïª§Ïä§?Ä ?¥ÌåÅ ?∏Îìú ?ïÎ¶¨
  ...
  function ensureRangeTooltip() {
    // ?´Î¶¨Î°úÎìú/?§ÌÅ¨Î¶ΩÌä∏ ?¨Ïã§???±ÏúºÎ°??®Ï? ?§Îûò???¥ÌåÅ ?∏ÎìúÎ•??ïÎ¶¨
    ...
    if (!rangeTooltipGlobalGuardBound) {
      document.addEventListener(
        ...
        (e) => {
          ...
          // ?îÍµ¨?¨Ìï≠: ?òÌèâÎ∞??ÅÏó≠ Î∞ñÏù¥Î©?ÎßàÏ?Îß??¥ÌåÅ ?¨Ìï® ?ÑÎ? Ï¶âÏãú ?úÍ±∞
          if (!hoveredLine) {
            hideRangeTooltipNow();
            return;
          }
  ...
  function showRangeTooltip(
    ...
    // ?¥Î≤§???ÑÎùΩÍ≥?Î¨¥Í??òÍ≤å "?òÌèâÎ∞îÎ? Î≤óÏñ¥?òÎ©¥ Ï¶âÏãú ?®Í?"??Í∞ïÏ†ú?úÎã§.
  ...
  function bindRangeLineTooltip(
    ...
    lineEl.addEventListener("mouseleave", () => {
      ...
      // ?îÍµ¨?¨Ìï≠: ?òÌèâÎ∞??ÅÏó≠??Î≤óÏñ¥?òÎ©¥ Ï¶âÏãú ?®Í?
      hideRangeTooltipNow();
    });
  ...
  function dateStrFromPoint(clientX, clientY) {
    ...
    // fallback: Ï¢åÌëúÍ∞Ä ?Ä Í≤ΩÍ≥Ñ???ÑÏóê Í±∏Ïπú Í≤ΩÏö∞ Î≥¥Ï°∞ ?êÏ†ï
  ...
  function bindRangeDrag(lineEl, task, anchorDateStr) {
    lineEl.addEventListener("pointerdown", (e) => {
      ...
      const finishMove = async (ev, cancelled) => {
        ...
        // ?úÎûòÍ∑??úÎ°≠ ÏßÅÌõÑ Î∞úÏÉù?òÎäî click ?¥Î≤§?∏Î? Î®ºÏ? ?µÏ†ú?úÎã§.
  ...
  /** ?ÑÎ£å?†ÏßúÍ∞Ä ?àÏúºÎ©?Í∑??†Ïßú Í∏∞Ï??ºÎ°ú ?ÑÎ£å. Íµ¨Î≤Ñ??doneÎß??àÏúºÎ©???Í∏∞Í∞Ñ ?ÑÎ£åÎ°?Î≥∏Îã§. */
  ...
  /** `.deliverable-editor`??`--deliverable-row-h`?Ä ?ôÏùº??Î∞îÍπ• ?íÏù¥(px). select/?ÖÎ†•???íÏù¥?Ä ÎßûÏ∂§. */
  ...
  /** ?∞Ï∂úÎ¨ºÎ™Ö: Ï§ÑÎ∞îÍø??êÎèô ?íÏù¥, ÏµúÎ? 5Ï§????§ÌÅ¨Î°?*/
  ...
  function setDeliverableInputRowOpen(open, opts = {}) {
    ...
    if (btnDeliverableAddToggle instanceof HTMLElement) {
      ...
      const label = open ? "?∞Ï∂úÎ¨??ÖÎ†•Ïπ??´Í∏∞" : "?∞Ï∂úÎ¨??ÖÎ†•Ïπ??¥Í∏∞";
  ...
  /**
   * ?ÖÎ†•????Î™©Î°ù?ºÎ°ú ?¥Î†§Í∞Ä???¥Îèô ?†ÎãàÎ©îÏù¥??
  ...
  /**
   * ?ÖÎ†•?âÏùò ?¥Ïö©???∞Ï∂úÎ¨?Î™©Î°ù??Ï∂îÍ??úÎã§.
  ...
  /** ?ÖÎ†•?âÏ? ?±Í∏â??Í≥†Î•¥Í∏??ÑÍπåÏßÄ ?åÏÉâ Î∞ïÏä§Î°??îÎã§. */
  ...
  function taskSpentDailyMhOnDate(task, dateStr) {
    ...
    if (!viewRows.length) {
      // ?∞Ï∂úÎ¨??ÜÏúºÎ©??ÑÎ£å ?ÅÌÉú???åÎßå ?πÏùº ?àÏÉÅÍ≥µÏàòÎ•??§Ï†Å?ºÎ°ú Î≥∏Îã§
    ...
    // Ï∫°Ï≤ò Í≥µÏãù: (?àÏÉÅMH / Ï§ëÏöî?ÑÌï©) √ó Í∑∏ÎÇ† ?ÑÎ£å???∞Ï∂úÎ¨?Ï§ëÏöî?????ÑÎ£å?ºÏóêÎß?Í∑Ä??    let spent = 0;
  ...
  const KR_FIXED_HOLIDAYS = [
    { md: "01-01", name: "?†Ï†ï", substitute: true },
    { md: "03-01", name: "?ºÏùº??, substitute: true },
    { md: "05-05", name: "?¥Î¶∞?¥ÎÇ†", substitute: true },
    { md: "06-06", name: "?ÑÏ∂©??, substitute: true },
    { md: "08-15", name: "Í¥ëÎ≥µ??, substitute: true },
    { md: "10-03", name: "Í∞úÏ≤ú??, substitute: true },
    { md: "10-09", name: "?úÍ???, substitute: true },
    { md: "12-25", name: "?±ÌÉÑ??, substitute: true },
  ];
  ...
  /**
   * ?¨Î†• ?Ä ?ôÍ∑∏?ºÎ??Ä ?ôÏùº??Î™©Î°ù¬∑?úÏÑú(ÏµúÎ? MAX_CALENDAR_DOTS).
   * @param {string} dateStr
   * @returns {Task[]}
   */
  ...
  function recurrenceLabel(rec) {
    if (rec === "daily") return "Îß§Ïùº";
    if (rec === "weekly") return "Îß§Ï£º";
    if (rec === "monthly") return "Îß§Ïõî";
    return "Î∞òÎ≥µ ?ÜÏùå";
  }

  /** ?¨Î†•: Î∞îÍπ• Îß?= Ï§ëÏöî???¨Í∏∞), ?àÏ™Ω ??= ÏßÑÌñâ ?ÅÌÉú ??*/
  ...
  async function checkAndHandleRecurrenceExtension() {
    ...
    const groups = [
      { rec: "daily", title: "Î∞òÎ≥µ ?∞Ïû• ?ïÏù∏", msg: "Îß§Ïùº Î∞òÎ≥µ???úÏ£º ?∞Ïû•?†Í±¥Í∞Ä??" },
      { rec: "weekly", title: "Î∞òÎ≥µ ?∞Ïû• ?ïÏù∏", msg: "Îß§Ï£º Î∞òÎ≥µ???úÎã¨ ?∞Ïû•?†Í±¥Í∞Ä??" },
      { rec: "monthly", title: "Î∞òÎ≥µ ?∞Ïû• ?ïÏù∏", msg: "Îß§Ïõî Î∞òÎ≥µ???ºÎÖÑ ?∞Ïû•?†Í±¥Í∞Ä??" },
    ];
  ...
  async function ensureFirebaseAnonymousAuth(app) {
    if (!window.firebase || !window.firebase.auth) {
      throw new Error("Firebase Auth SDKÍ∞Ä Î°úÎìú?òÏ? ?äÏïò?µÎãà??");
    }
  ...
  async function loadTasks() {
    if (!window.firebase || !window.firebase.apps) {
      console.error("Firebase SDKÍ∞Ä Î°úÎìú?òÏ? ?äÏïò?µÎãà??");
      return;
    }
    ...
    try {
      ...
      alert(
        "Firebase ?µÎ™Ö Î°úÍ∑∏?∏Ïóê ?§Ìå®?àÏäµ?àÎã§.\n" +
          "Firebase Console ??Authentication ??Sign-in method ?êÏÑú Anonymous Î•??úÏÑ±?îÌï¥ Ï£ºÏÑ∏??\n\n" +
    ...
    firebaseTasksRef.on(
      ...
      (err) => {
        ...
        alert(
          "Firebase ?∞Ïù¥???ôÍ∏∞?îÏóê ?§Ìå®?àÏäµ?àÎã§.\n" +
            "Realtime Database Rules ?êÏÑú auth != null ????shared-calendar ?ΩÍ∏∞/?∞Í∏∞Î•??àÏö©??Ï£ºÏÑ∏??\n\n" +
            (err && err.message ? err.message : String(err))
        );
      }
    );

    // Î©îÌ? Í≤ΩÎ°úÍ∞Ä ?ÜÎçî?ºÎèÑ ?ùÏÑ±?????àÎèÑÎ°?no-op write Î≥¥Ïû•
  ...
  async function waitForFirebaseReady(timeoutMs = 8000) {
    ...
    while (!firebaseDb || !firebaseTasksRef) {
      if (Date.now() - start > timeoutMs) {
        throw new Error("Firebase ?∞Í≤∞???ÑÏßÅ Ï§ÄÎπÑÎêòÏßÄ ?äÏïò?µÎãà?? ?†Ïãú ???§Ïãú ?úÎèÑ??Ï£ºÏÑ∏??");
      }
  ...
  function applyCalendarRowHeights(
    ...
    for (let row = 0; row < rowCount; row++) {
      ...
      inRow.forEach((cell) => {
        ...
        // ?ÄÎ≥??§Ï†ú ?§Îçî ?òÎã®(?†Ïßú ?´Ïûê/Í≥µÌú¥?ºÎ™Ö/?∞Ï∏° Î∞∞Ï?)??Ï∏°Ï†ï??        // ?òÌèâÎ∞??§ÌÉù + ?òÎã® dot ?ÅÏó≠ Î≥¥Ìò∏ ?íÏù¥Î•?Í≥ÑÏÇ∞?úÎã§.
        const numEl = /** @type {HTMLElement | null} */ (cell.querySelector(".calendar-cell__num"));
        const holidayEl = /** @type {HTMLElement | null} */ (cell.querySelector(".calendar-cell__holiday-name"));
        const badgeEl = /** @type {HTMLElement | null} */ (cell.querySelector(".calendar-cell__ongoing-count"));
        const numBottom = numEl ? numEl.offsetTop + numEl.offsetHeight : 0;
        ...
        // ?†ÏßúÎ≥?dot Í∞úÏàò(1Ï§?2Ï§?3Ï§????∞Îùº ?§Ï†ú ?òÎã® Î≥¥Ìò∏ ?íÏù¥Î•?Î∞òÏòÅ?úÎã§.
      ...
      // ?ôÍ∑∏?ºÎ???"ÎßàÏ?Îß??òÌèâÎ∞??òÎã® + ?¨Ïú†" ?ÑÎûò?êÏÑú ?úÏûë?¥Ïïº ?úÎã§.
  ...
  function renderDeliverableList(rows) {
    ...
    sortDeliverablesForView(normalizeDeliverables(rows)).forEach((row) => {
      ...
      dateInput.title = "?∞Ï∂úÎ¨??ùÏÇ∞?ÑÎ£å?ºÏûê";
      dateInput.setAttribute("aria-label", "?∞Ï∂úÎ¨??ùÏÇ∞?ÑÎ£å?ºÏûê");
      ...
      nameTa.placeholder = "?∞Ï∂úÎ¨ºÎ™Ö";

      const impSelect = document.createElement("select");
      impSelect.className = "deliverable-item__importance-select";
      impSelect.innerHTML = `<option value="high">??/option><option value="medium">Ï§?/option><option value="low">??/option>`;
  ...
  function updateSearchResults() {
    ...
    searchCount.textContent = `(${hits.length}Í±?`;
    searchResultsList.innerHTML = "";
    ...
    if (hits.length === 0) {
      ...
      li.textContent = "Ï°∞Í±¥??ÎßûÎäî ?ºÏ†ï???ÜÏäµ?àÎã§.";
    ...
    hits.forEach((t) => {
      ...
      const rec =
        t.recurrence && t.recurrence !== "none"
          ? ` ¬∑ Î∞òÎ≥µ: ${recurrenceLabel(t.recurrence)}${t.recurrenceUntil ? " ~ " + t.recurrenceUntil : ""}`
          : "";
      btn.innerHTML = `<div class="search-hit__title">${escapeHtml(lab)}</div><div class="search-hit__meta">${escapeHtml(t.status)} ¬∑ Ï§ëÏöî??${escapeHtml(importanceLabel(t.importance))} ¬∑ ${t.startDate} ~ ${t.endDate}${escapeHtml(rec)}</div>`;
  ...
  function buildDailyMhBadgeTooltip(dayEffortRows, spentDailyMh, totalDailyMh) {
    ...
    let tip = `?πÏùº Í≥µÏàò ${formatMh(spentDailyMh)}/${formatMh(totalDailyMh)}MH (?¨ÏûÖ/?àÏÉÅ)`;
    tip += "\n?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä\n";
    tip += shown.map((lab, i) => `${i + 1}. ${lab}`).join("\n");
    if (lines.length > maxLines) tip += `\n????${lines.length - maxLines}Í±?;
    return tip;
  }
  ...
  function styleCellForDate(cell, dateStr) {
    ...
    if (ongoingEl) {
      if (totalDailyMh > 0) {
        ...
        ongoingEl.setAttribute(
          "aria-label",
          `${dateStr} ?πÏùº Í≥µÏàò ${formatMh(spentDailyMh)}/${formatMh(totalDailyMh)}MH (?¨ÏûÖ/?àÏÉÅ): ${allEffortRows.map((x) => taskLabel(x.task)).join(", ")}`
        );
      } else {
    ...
    if (dots && list.length) {
      ...
      sorted.forEach((t) => {
        ...
        // ?ôÍ∑∏?ºÎ???Ïª§Ïä§?Ä ?åÎ°ú???¥ÌåÅ???∞Ï? ?äÎäî??
        // (?òÌèâÎ∞??¥ÌåÅ ?îÎ•ò/?¨Ìä∏Î¶¨Í±∞?Ä Ï∂©Îèå Î∞©Ï?)
        // ?ôÍ∑∏?ºÎ? ?¥Î¶≠ ???¥Îãπ taskÎ•?Î∞îÎ°ú ?∏Ïßë Î™®ÎìúÎ°??∞Îã§.
      ...
      if (fullSorted.length > MAX_CALENDAR_DOTS) {
        ...
        more.title = `??${fullSorted.length - MAX_CALENDAR_DOTS}Í±?;
  ...
  function openModal(dateStr, taskId, forceNew = false) {
    ...
    if (taskId) {
      ...
      // ?¥Îãπ ?†Ïßú???ºÏ†ï???àÏúºÎ©?Ï≤???™©??Í∏∞Î≥∏ ?†ÌÉù(?†Ïßú Î≥ÄÍ≤?Ï¶âÏãú ?∏Ïßë Í∞Ä??
      const onDay = tasks
  ...
  function renderExistingTasksList() {
    ...
    onDaySorted.forEach((t) => {
      ...
      btn.title = "?¥Î¶≠: ?ÅÏÑ∏ ?∏Ïßë ¬∑ ?∏Î≤Ñ: ?ÅÌÉú/Ï§ëÏöî????†ú";
      btn.addEventListener("click", () => {
        editingId = t.id;
        modalDefaultWhite = false;
        taskTitle.value = t.title || "";
      ...
      statusRow.innerHTML = `<span class="task-chip__quick-label">?ÅÌÉú</span>`;
      ...
      del.addEventListener("click", async (e) => {
        e.preventDefault();
        e.stopPropagation();
        const ok = await openConfirmDialog(`"${taskLabel(t)}" ?ºÏ†ï????†ú?†Íπå??`, {
          title: "??†ú ?ïÏù∏",
          okLabel: "??†ú",
          cancelLabel: "Ï∑®ÏÜå",
          showCancel: true,
  ...
  async function openAlertDialog(message, title = "?ÖÎ†• ?ïÏù∏") {
  ...
  async function loadGeminiKey() {
    // ?ÑÏ†Ñ ÎπÑÏ???Î™®Îìú: ?§ÎèÑ Î©îÎ™®Î¶¨Ïóê?úÎßå ?¨Ïö©
  ...
  function parseGeminiJsonArray(raw) {
    ...
    throw new Error("JSON Î∞∞Ïó¥ ?ïÏãù???ÑÎãô?àÎã§.");
  }

  /**
   * OCR ?†Ïßú Î¨∏Ïûê?¥ÏùÑ YYYY-MM-DDÎ°??ïÍ∑ú?îÌïú??
   * - YYYY-MM-DD
   * - YYYY/M/D, YYYY.M.D
   * - M-D, M/D, M.D, M??D?? -> fallbackYear ?¨Ïö©
   * @param {unknown} raw
  ...
  /**
   * @param {string} base64 ??Data URL ???ÑÎãå ?úÏàò base64
   * @param {string} mimeType
   * @param {string} apiKey
   */
  async function runGeminiOcr(base64, mimeType, apiKey) {
    const todayStr = toDateStrFromDate(new Date());
    const prompt = `???¥Î?ÏßÄ???úÍ?Î°??ÅÌûå ?†Ïùº Î™©Î°ù(?êÍ????êÎäî ?∏ÏáÑ)?ÖÎãà?? Î™®Îì† ?†Ïùº???ΩÏñ¥ JSON Î∞∞Ïó¥Îß?Ï∂úÎ†•?òÏÑ∏??

?§ÌÇ§Îß? Í∞??êÏÜå??{"title": string, "startDate": "YYYY-MM-DD", "endDate": "YYYY-MM-DD", "status": "ready"|"on-going"|"done", "importance": "high"|"medium"|"low", "effortValue": number, "effortUnit": "MH"|"MD", "description": string, "confidence": number}
Í∑úÏπô:
- ?†ÏßúÍ∞Ä ?ÅÌ? ?àÏúºÎ©?Í∑??†ÏßúÎ•??¨Ïö©?©Îãà?? Í∞ôÏ? ?†Ïßú ?ÑÎûò???¨Îü¨ Ï§ÑÏù¥ ?àÏúºÎ©?Í∞ÅÍ∞Å Î≥ÑÎèÑ ??™©?ºÎ°ú ?êÍ≥† Í∞ôÏ? startDate?Ä endDateÎ•??ÅÎãà??
- ?∞ÎèÑÍ∞Ä ?ÜÎäî ?†Ïßú(?? 5/4, 5??4?????ÑÏû¨ ?¨Î†• ?îÎ©¥???∞ÎèÑ(${viewYear})Î•?Î∂ôÏó¨ YYYY-MM-DDÎ°?ÎßåÎì≠?àÎã§.
- ?†ÏßúÍ∞Ä ?ÑÌ? ?ÜÏúºÎ©?startDate/endDate??Îπ?Î¨∏Ïûê?¥Î°ú ?°Îãà??
- ??Ï§ÑÏóê ?†Ïßú?Ä ?úÎ™©??Í∞ôÏù¥ ?àÏúºÎ©?Í∑??†Ïßú??Í∑??úÎ™©???£Ïäµ?àÎã§.
- status???êÎ≥Ñ Í∞Ä?•Ìï† ??Ï±ÑÏö∞Í≥? ?†Îß§?òÎ©¥ "ready"Î°??°Îãà??
- importance???êÎ≥Ñ Í∞Ä?•Ìï† ?åÎßå Ï±ÑÏö∞Í≥? ?†Îß§?òÎ©¥ "medium"?ºÎ°ú ?°Îãà??
- effortValue/effortUnit?Ä ?ÅÌ? ?àÏùÑ ??Ï±ÑÏö∞Í≥? ?ÜÏúºÎ©?effortValue??4, effortUnit?Ä "MH"Î°??°Îãà??
- description?Ä Î∂ÄÍ∞Ä Î©îÎ™®Í∞Ä ?àÏùÑ ?åÎßå Ï±ÑÏö∞Í≥??ÜÏúºÎ©?Îπ?Î¨∏Ïûê??
- confidence???¥Îãπ ??™© ?∏Ïãù ?†Î¢∞??0~1 ?êÎäî 0~100 ?´Ïûê)Î°??£Ïäµ?àÎã§.
- JSON Î∞∞Ïó¥Îß?Ï∂úÎ†•?òÍ≥† ?§Î•∏ ?§Î™Ö?Ä ?∞Ï? ÎßàÏÑ∏??`;
    ...
    for (const model of candidates) {
      ...
      if (!res.ok) {
        ...
        // ?ºÎ? Î™®Îç∏?Ä responseMimeType ÏßÄ?êÏù¥ ?úÌïú?????àÏñ¥ ??Î≤???plain ?∏Ï∂ú
      ...
      if (!cand) {
        lastErr = `[${model}] ?ëÎãµ???ÑÎ≥¥Í∞Ä ?ÜÏäµ?àÎã§.`;
        continue;
      }
      ...
      if (text == null || String(text).trim() === "") {
        lastErr = `[${model}] ?ëÎãµ ?çÏä§?∏Í? ÎπÑÏñ¥ ?àÏäµ?àÎã§.`;
        continue;
      }
      const arr = parseGeminiJsonArray(String(text));
      return arr.map((o) => coerceOcrItem(o, todayStr));
    }
    throw new Error(lastErr || "?¨Ïö© Í∞Ä?•Ìïú Gemini Î™®Îç∏??Ï∞æÏ? Î™ªÌñà?µÎãà??");
  }
  ...
  function renderOcrDraftList() {
    ...
    ocrDraftRows.forEach((row, index) => {
      ...
      li.innerHTML = `
        <label class="ocr-draft-check"><input type="checkbox" class="ocr-draft-cb" checked data-index="${index}" /> ?¨Ìï®</label>
        <div class="ocr-draft-fields">
          <div class="ocr-draft-confidence" title="OCR ?†Î¢∞??>?†Î¢∞??${Math.max(1, Math.min(99, Math.round(Number(row.confidence || 0))))}%</div>
          <input type="text" class="toolbar__input ocr-draft-title" data-index="${index}" id="${idPrefix}-title" />
          <div class="ocr-draft-dates">
            <label class="ocr-draft-date-lab">?úÏûë <input type="date" class="ocr-draft-start" data-index="${index}" id="${idPrefix}-s" /></label>
            <label class="ocr-draft-date-lab">?ÑÎ£å <input type="date" class="ocr-draft-end" data-index="${index}" id="${idPrefix}-e" /></label>
          </div>
          <div class="ocr-draft-meta">
            <label>?ÅÌÉú
              <select class="ocr-draft-status" data-index="${index}">
                <option value="">(ÎπàÏπ∏)</option>
                <option value="ready">ready</option>
                <option value="on-going">on-going</option>
                <option value="done">done</option>
              </select>
            </label>
            <label>Ï§ëÏöî??              <select class="ocr-draft-imp" data-index="${index}">
                <option value="">(ÎπàÏπ∏)</option>
                <option value="high">??/option>
                <option value="medium">Ï§?/option>
                <option value="low">??/option>
              </select>
            </label>
            <label>Í≥µÏàò
              <input type="number" class="ocr-draft-effort" data-index="${index}" min="0" step="0.25" placeholder="ÎπàÏπ∏ Í∞Ä?? />
            </label>
            <label>?®ÏúÑ
          ...
          <textarea class="ocr-draft-desc" rows="2" data-index="${index}" placeholder="?§Î™Ö(?†ÌÉù)"></textarea>
        </div>`;
  ...
  function readOcrDraftFromDom() {
    ...
    ocrDraftRows.forEach((_, index) => {
      ...
      const payload = {
        title: (title && title.value.trim()) || "(?úÎ™© ?ÜÏùå)",
  ...
  function openCameraCapture() {
    ...
    if (!("mediaDevices" in navigator)) {
      openAlertDialog("??Í∏∞Í∏∞/Î∏åÎùº?∞Ï???Ïπ¥Î©î??Ï¥¨ÏòÅ??ÏßÄ?êÌïòÏßÄ ?äÏäµ?àÎã§.");
      return;
    }
    openOcrModal();
  ...
  async function saveFromModal() {
    // ?ÖÎ†•Ïπ∏Ïóê ?®ÏïÑ ?àÎäî ?∞Ï∂úÎ¨ºÎèÑ ?Ä???Ä?ÅÏóê ?¨Ìï®?úÎã§.
    ...
    if (effortInputRaw === "") {
      await openAlertDialog("?¨ÏûÖ?àÏÉÅÍ≥µÏàòÎ•??ÖÎ†•??Ï£ºÏÑ∏??");
    ...
    // ????™©?êÏÑú ?úÎ™©/?§Î™Ö??Î™®Îëê ÎπÑÎ©¥ ?Ä?•ÌïòÏßÄ ?äÍ≥† ?´Îäî??(?†Î†π ??™© ?ùÏÑ± Î∞©Ï?)
    ...
    if (taskEffortValue.value.trim() !== "" && (!Number.isFinite(effortRaw) || effortRaw <= 0)) {
      await openAlertDialog("?¨ÏûÖ?àÏÉÅÍ≥µÏàò??0Î≥¥Îã§ ???´ÏûêÎ°??ÖÎ†•??Ï£ºÏÑ∏??");
      return;
    }

    if (!startDate || !endDate) {
      await openAlertDialog("?úÏûë?ºÍ≥º ?ÑÎ£å?ºÏùÑ Î™®Îëê ?†ÌÉù??Ï£ºÏÑ∏??");
      return;
    }
    if (parseDateStr(startDate) > parseDateStr(endDate)) {
      await openAlertDialog("?úÏûë?ºÏù¥ ?ÑÎ£å?ºÎ≥¥????ùÑ ???ÜÏäµ?àÎã§.");
      return;
    }
  ...
  async function deleteTask() {
    if (!editingId) return;
    const ok = await openConfirmDialog("???ºÏ†ï????†ú?†Íπå?? Î∞òÎ≥µ ?ºÏ†ï?¥Î©¥ ?ÑÏ≤¥ ?úÎ¶¨Ï¶àÍ? ??†ú?©Îãà??", {
      title: "??†ú ?ïÏù∏",
      okLabel: "??†ú",
      cancelLabel: "Ï∑®ÏÜå",
      showCancel: true,
  ...
  /* ?Ä?Ä ?úÏûë??Ï¢ÖÎ£å?? ÎßàÏö∞?§Î? ?¨Î¶¨Î©??®Îäî ?¨Î†•(?úÎûòÍ∑∏Î°ú Í∏∞Í∞Ñ ÏßÄ?? ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä */
  ...
  /** ÎßàÏö∞???¥Î¶≠ ÏßÅÌõÑ??focus Î°úÎäî ?¨Î†•???§Ïãú ?¥Ï? ?äÎäî??Í∏∞Î≥∏ ?¨Î†•Í≥?Ï∂©Îèå Î∞©Ï?). */
  ...
  function ensureDatePop() {
    ...
    pop.setAttribute("aria-label", "?†Ïßú ?†ÌÉù");
    pop.innerHTML = `
      <div class="date-pop__head">
        <button type="button" class="date-pop__nav" data-nav="-1" aria-label="?¥Ï†Ñ ??>??/button>
        <span class="date-pop__title"></span>
        <button type="button" class="date-pop__nav" data-nav="1" aria-label="?§Ïùå ??>??/button>
      </div>
      <div class="date-pop__weekdays">
        ...
        <span class="date-pop__weekday">??/span>
        <span class="date-pop__weekday">??/span>
        <span class="date-pop__weekday">Î™?/span>
        <span class="date-pop__weekday">Í∏?/span>
        <span class="date-pop__weekday date-pop__weekday--sat">??/span>
      </div>
      <div class="date-pop__grid"></div>
      <p class="date-pop__hint">?¥Î¶≠: ?†Ïßú ÏßÄ??¬∑ ?úÎûòÍ∑? ?úÏûë??Ï¢ÖÎ£å??ÏßÄ??/p>
    `;
    ...
    if (datePopGrid) {
      datePopGrid.addEventListener("mousedown", (e) => {
        ...
        // ???¥Î≤§?∏Í? Î¨∏ÏÑúÍπåÏ? ?ÑÌåå?òÎäî ?ôÏïà ?åÎ¶∞ Î≤ÑÌäº???¨ÎùºÏßÄÏßÄ ?äÎèÑÎ°??§Ïùå ?ÑÎ†à?ÑÏóê ?§Ïãú Í∑∏Î¶∞??
        requestAnimationFrame(() => {
      ...
      datePopGrid.addEventListener("mouseover", (e) => {
        ...
        // ?§Ïùå???êÎäî ?¥Ï†Ñ?? Ïπ∏ÏúºÎ°??úÎûòÍ∑∏ÌïòÎ©??¥Îãπ ?¨Î°ú Î∑∞Î? ?òÍ≤® ?∞ÏÜç ?†ÌÉù???òÍ≤å ?úÎã§
  ...
  function renderDatePop() {
    ...
    // ?¥Ï†Ñ???†ÏßúÎ•?Ï±ÑÏõå ?îÎßê?íÏõîÏ¥??úÎûòÍ∑∏Í? ?äÍ∏∞ÏßÄ ?äÍ≤å ?úÎã§
  ...
  /** ?¨Î†•?êÏÑú Í≥†Î•∏ Í∞íÏùÑ ?ÖÎ†•Ïπ∏Ïóê Î∞òÏòÅ?úÎã§. */
  ...
  [
    ...
    // Ïª§Ïä§?Ä ?¨Î†•???∞Î?Î°??§Ïù¥?∞Î∏å date picker??ÎßâÍ≥†, ?¥Î¶≠?¥ÎèÑ ?ùÏóÖ???´Ï? ?äÎäî??    input.addEventListener("mousedown", (e) => {
      e.preventDefault();
  ...
  document.addEventListener("mouseup", () => {
    ...
    // ?òÏ†ï ?úÎûòÍ∑∏Î? ?¥Ïñ¥???????àÎèÑÎ°??ùÏóÖ?Ä ?†Ï?
    renderDatePop();
  });
  ...
  taskStart.addEventListener("change", () => {
    if (taskStart.value && taskEnd.value && parseDateStr(taskStart.value) > parseDateStr(taskEnd.value)) {
      openAlertDialog("?úÏûë?ºÏù¥ ?ÑÎ£å?ºÎ≥¥????ùÑ ???ÜÏäµ?àÎã§.");
  ...
  if (btnDeliverableAddToggle) {
    btnDeliverableAddToggle.addEventListener("click", () => {
      if (isDeliverableInputRowOpen()) {
        // C2: ?å‚àí???ÑÎ•¥Î©??ÖÎ†•Ï∞ΩÏùÑ ?´Í≥† ?ÅÏñ¥ ???¥Ïö©??Ï¥àÍ∏∞?îÌïú??(?±Î°ù?òÏ? ?äÏùå).
        resetDeliverableInputRow();
        return;
      }
      setDeliverableInputRowOpen(true, { focus: true });
  ...
  // Î™®Îã¨ ???§Î•∏ Í≥≥ÏùÑ ?ÑÎ•¥Î©? ?ÖÎ†• Ï§ëÏù¥???∞Ï∂úÎ¨ºÏùÑ Î™©Î°ù?ºÎ°ú ?¥Îèô ?†ÎãàÎ©îÏù¥?????ÖÎ†•Ï∞ΩÏùÑ ?´Îäî??
  ...
  if (btnExport && btnImport && importFileInput) {
    ...
    importFileInput.addEventListener("change", async () => {
      ...
      try {
        ...
        if (!incoming) {
          alert("?ºÏ†ïÎ≥µÏõê ?åÏùº ?ïÏãù???¨Î∞îÎ•¥Ï? ?äÏäµ?àÎã§. (tasks Î∞∞Ïó¥ ?ÑÏöî)");
          return;
        }
        const ok = await openConfirmDialog("?ÑÏû¨ ?ºÏ†ï??Î∞±ÏóÖ ?åÏùº ?¥Ïö©?ºÎ°ú ??ñ¥?∏Íπå??", {
          title: "?ºÏ†ïÎ≥µÏõê ?ïÏù∏",
          okLabel: "?ºÏ†ïÎ≥µÏõê",
          cancelLabel: "Ï∑®ÏÜå",
          showCancel: true,
        });
        ...
        renderCalendar();
        updateSearchResults();
        alert(`?ºÏ†ïÎ≥µÏõê ?ÑÎ£å: ${tasks.length}Í±?);
      } catch (e) {
        alert("?ºÏ†ïÎ≥µÏõê ?§Ìå®: JSON ?åÏùº???ïÏù∏??Ï£ºÏÑ∏??");
      }
    });
  }
  ...
  if (
    ...
    btnSaveGeminiKey.addEventListener("click", () => {
      ...
      ocrStatus.textContent = "API ?§Î? ??Î∏åÎùº?∞Ï????Ä?•Ìñà?µÎãà??";
    });
    ...
    btnOcrRun.addEventListener("click", async () => {
      ...
      if (!key) {
        ocrStatus.textContent = "API ?§Î? ?ÖÎ†•?òÍ±∞???åÌÇ§ ?Ä?•„ÄçÏùÑ ?åÎü¨ Ï£ºÏÑ∏??";
        return;
      }
      if (!ocrPendingBase64) {
        ocrStatus.textContent = "?¥Î?ÏßÄÎ•?Î®ºÏ? ?†ÌÉù??Ï£ºÏÑ∏??";
        return;
      }
      ocrStatus.textContent = "?∏Ïãù Ï§ë‚Ä?;
      btnOcrRun.disabled = true;
      try {
        ocrDraftRows = await runGeminiOcr(ocrPendingBase64, ocrPendingMime, key);
        if (!ocrDraftRows.length) {
          ocrStatus.textContent = "?∏Ïãù???†Ïùº???ÜÏäµ?àÎã§. ?§Î•∏ ?¨ÏßÑ?ºÎ°ú ?úÎèÑ??Î≥¥ÏÑ∏??";
          ocrResults.hidden = true;
          return;
        }
        renderOcrDraftList();
        ocrResults.hidden = false;
        ocrStatus.textContent = `${ocrDraftRows.length}Í±¥ÏùÑ ?∏Ïãù?àÏäµ?àÎã§. ?òÏ†ï ???åÏÑ†????™© ?¨Î†•??Ï∂îÍ??çÎ? ?ÑÎ•¥?∏Ïöî.`;
      } catch (e) {
        const msg = e instanceof Error ? e.message : String(e);
        if (/api key not valid|permission|referer|denied|forbidden|401|403/i.test(msg)) {
          ocrStatus.textContent =
            "?§Î•ò: API ??Í∂åÌïú/?úÌïú Î¨∏Ï†ú?ÖÎãà?? Google AI Studio?êÏÑú ???úÌïú(HTTP Î¶¨Ìçº??IP) ?êÎäî ?ÑÎ°ú?ùÌä∏ Í∂åÌïú???ïÏù∏??Ï£ºÏÑ∏??";
        } else if (/quota|rate|429/i.test(msg)) {
          ocrStatus.textContent = "?§Î•ò: ?¨Ïö©???úÎèÑ(Quota) Ï¥àÍ≥º?ÖÎãà?? ?†Ïãú ???§Ïãú ?úÎèÑ?òÍ±∞??Í≤∞Ï†ú/ÏøºÌÑ∞Î•??ïÏù∏??Ï£ºÏÑ∏??";
        } else {
          ocrStatus.textContent = "?§Î•ò: " + msg;
        }
        ocrResults.hidden = true;
      } finally {
        btnOcrRun.disabled = false;
    ...
    btnOcrApply.addEventListener("click", async () => {
      ...
      try {
        ...
        renderCalendar();
        ocrStatus.textContent = `?Ä???ÑÎ£å: ${newTasks.length}Í±?;
        closeOcrModal();
      } catch (e) {
        tasks = prevTasks;
        renderCalendar();
        ...
        const msg = e instanceof Error ? e.message : String(e);
        ocrStatus.textContent = "?Ä???§Ìå®: " + msg;
        alert("?¨Î†• ?Ä??Ï§??§Î•òÍ∞Ä Î∞úÏÉù?àÏäµ?àÎã§: " + msg);
      } finally {
  ...
  // Î™®Îã¨ Î∞îÍπ• ?¥Î¶≠ = ?ÑÎ£å: Î≥ÄÍ≤ΩÏù¥ ?àÏúºÎ©??Ä?•ÌïòÍ≥? ?ÜÏúºÎ©?Í∑∏ÎÉ• ?´Îäî??
  ...
  /** PWA: ?úÎπÑ???åÏª§ ?±Î°ù (file:// Î°??¥Ïóà???åÎäî Í±¥ÎÑà?Ä) */
